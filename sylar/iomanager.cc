...
int IOManager::addEvent(int fd,Event event,std::function<void()>cb){
    ...
    //将新的事件加入到epoll_wait,使用epoll_wait的私有指针存储FdContext的位置
    ...
    //赋值scheduler和回调函数,如果回调函数为空,则把协程当成回调执行体
...
bool IOManager::dekEvent(int fd,Event event){
    // 找到fd对应的FdContext
    RWMutexType::ReadLock lock(m_mutex);
    if ((int)m_fdContexts.size() <= fd) {
        return false;
    ...
    //清除指定的事件,表示不关心这个时间了,如果清楚之后结果为0,则从epoll_wait中删除文件描述符
